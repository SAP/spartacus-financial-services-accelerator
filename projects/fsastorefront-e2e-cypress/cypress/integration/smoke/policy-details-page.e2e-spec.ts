...
context('Policy Details Page', () => {
  ...
  it('Should have 6 accordions with correct titles and first one opened', () => {
    cy.get('fsa-accordion-item').should(
      'have.length',
      policyDetailsAccordions.length
    );
    ...
    cy.get('fsa-accordion-item').each((item, index, list) => {
      cy.get('fsa-accordion-item')
        ...
        .should(
          'have.attr',
          'ng-reflect-title',
          policyDetailsAccordions[index].title
        );
    });
  });
    ...
    cy.get('fsa-accordion-item').each((item, index, list) => {
      cy.get('fsa-accordion-item')
        ...
        .within(() => {
          cy.get('li').should(
            'have.length',
            policyDetailsAccordions[index].items.length
          );
            ...
            cy.get('li')
              ...
              .within(() => {
                cy.get('.item-label').should(
                  'have.text',
                  policyDetailsAccordions[index].items[index1].label
                );
                cy.get('.value').should(
                  'have.text',
                  policyDetailsAccordions[index].items[index1].value
                );
              });
          });
        });
